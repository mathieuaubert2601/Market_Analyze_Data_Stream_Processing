...
def display_stock_chart(ticker: str) -> None:
    ...
    try:
        ...
        # ✅ La première colonne est l'index (date)
        ...
        # ✅ Normaliser et convertir les colonnes en numériques
        ...
        if data.empty: 
            st. warning("📊 CSV vide ou données invalides")
            return
...
with col1:
    ...
    if submit and query:
        with st.spinner("🧠 Agent is analyzing Kafka streams..."):
            ...
            mode_label = "🔴 LIVE MODE" if horizon_hours < 4 else "📚 HISTORY MODE"
        ...
        for s in st.session_state.sources:
            ...
            # --- NEWS SOURCE ---
                ...
                with st.expander(f"{icon} {s['ticker']} - News ({relative_time})"):
                    ...
                    st.divider()
                    if s.get('link') and s['link'] != "#":
                        st.markdown(f"[📖 Read full article]({s['link']})")
